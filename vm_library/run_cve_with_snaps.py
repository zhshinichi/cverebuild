...
from qemu_utils import create_overlay, get_qemu_cmd, savevm_snapshot
...
def main():
    ...
    subprocess.run([
        "ansible-playbook", "-i", inv_path, "--extra-vars", f'test_case=\'{cve}\' cve_json=\'{cve_json}\' run_type=\'{run_type}\'',
        playbook
    ], check=True)
    
    print(f"✅ CVE run part A complete")
    
    #SNAPSHOT
    savevm_snapshot(f"/tmp/qemu-monitor-socket-{ssh_port}", f"{cve}_snap_part_A")
    ...
    # # PART B
    # qemu_cmd = get_qemu_cmd(cpus, memory, output_image, ssh_port, shared_dir, init=False, snapshot_name=f"{cve}_snap_part_A")
    ...
    # print(f"✅ CVE run part B complete")

    # # SNAPSHOT
    # # savevm_snapshot(f"/tmp/qemu-monitor-socket-{ssh_port}", f"{cve}_snap_part_B")
    ...
    # #PART B Again?
    # qemu_cmd = get_qemu_cmd(cpus, memory, output_image, ssh_port, shared_dir, init=False, snapshot_name="snap_part_A")
    ...
    # print(f"Running Ansible playbook {playbook} for cve {cve} in {cve_json}")
    # subprocess.run([
    #     "ansible-playbook", "-i", inv_path, "--extra-vars", f'test_case=\'{cve}\' cve_json=\'{cve_json}\'',
    #     playbook
    # ], check=True)
    ...
    # print(f"✅ CVE run part B Again complete")
